...
TRAIN_DIR = '/home/arjun/MS/Thesis/CAMELYON-16/Data/Processed/training'
...
# def _eval_once(saver, summary_writer, accuracy, summary_op, confusion_matrix_op, true_positive_op, true_negative_op,
#                false_positive_op, false_negative_op, logits, labels, dense_labels):
    ...
    with tf.Session() as sess:
        ...
        try:
            ...
            while step < num_iter and not coord.should_stop():
                # correct_count, confusion_matrix, true_positive, true_negative, false_positive, \
                # false_negative, logits_v, labels_v, dense_labels_v = sess.run([accuracy, confusion_matrix_op,
                #                                                                true_positive_op, true_negative_op,
                #                                                                false_positive_op, false_negative_op,
...
def evaluate(dataset):
    ...
    with tf.Graph().as_default():
        ...
        # true_positive_op = metrics.streaming_true_positives(logits, dense_labels)
        # false_positive_op = metrics.streaming_false_positives(logits, dense_labels)
        # true_negative_op = metrics.streaming_true_negatives(logits, dense_labels)
        ...
        while True:
            ...
            # _eval_once(saver, summary_writer, accuracy, summary_op, confusion_matrix_op, true_positive_op,
            #            true_negative_op, false_positive_op, false_negative_op, logits, labels, dense_labels)
            if FLAGS.run_once:
                break
