...
def copy_all(from_dir, to_dir, file_names):
...
def move_all(from_dir, to_dir, file_names):
    total_file_count = len(file_names)
...
def move_files(from_dir, to_dir, n_sample=None):
    print('move_files() - From: %s' % from_dir)
    print('move_files() -   To: %s' % to_dir)
    file_names = os.listdir(from_dir)
    file_names = sorted(file_names)
    if n_sample is None:
        move_all(from_dir, to_dir, file_names)
    else:
...
def delete_all(from_dir, file_names, name_patter=None):

    if name_patter is not None:
        print('deleting files with pattern: %s' % name_patter)
        file_paths = glob.glob(os.path.join(from_dir, name_patter))
...
def delete_files(from_dir, name_patter=None, n_sample=None):
    print('delete_files() - From: %s' % from_dir)
    file_names = os.listdir(from_dir)
    file_names = sorted(file_names)
    if n_sample is None:
        delete_all(from_dir, file_names, name_patter)
    else:
        assert len(file_names) >= n_sample, "Not enough files to delete. available: %d, requested: %d" % \
                                            (len(file_names), n_sample)
        if name_patter is None:
            shuffled_index = list(range(len(file_names)))
            random.seed(12345)
            random.shuffle(shuffled_index)
            for index in range(n_sample):
...
# copy_files(utils.PATCHES_TRAIN_AUG_POSITIVE_PATH, utils.PATCHES_TRAIN_POSITIVE_PATH)
# delete_files(utils.PATCHES_VALIDATION_NEGATIVE_PATH)
# delete_files(utils.PATCHES_VALIDATION_POSITIVE_PATH)
...
# move_files(utils.PATCHES_TRAIN_POSITIVE_PATH, utils.PATCHES_VALIDATION_POSITIVE_PATH, n_sample=5000)
