...
class PatchExtractor(object):
    ...
    def extract_positive_patches_from_tumor_region(wsi_image, tumor_gt_mask, level_used,
        ...
        for bounding_box in bounding_boxes:
            ...
            X = np.random.random_integers(b_x_start, high=b_x_end, size=1500)
            Y = np.random.random_integers(b_y_start, high=b_y_end, size=1500)
    ...
    def extract_negative_patches_from_normal_wsi(wsi_image, image_open, level_used,
        ...
        for bounding_box in bounding_boxes:
            ...
            b_x_end = int(bounding_box[0]) + int(bounding_box[2])
            b_y_end = int(bounding_box[1]) + int(bounding_box[3])
            X = np.random.random_integers(b_x_start, high=b_x_end, size=750)
            Y = np.random.random_integers(b_y_start, high=b_y_end, size=750)
...
class WSIOps(object):
    ...
    def find_roi_bb_tumor_gt_mask(self, mask_image):
    ...
    def find_roi_bb_normal(self, rgb_image):
        ...
        bounding_boxes, rgb_contour = self.get_bb_normal(image_open, rgb_image)
        return bounding_boxes, rgb_contour, image_open

    def find_roi_bb_tumor(self, rgb_image, tumor_gt_mask):
        hsv = cv2.cvtColor(rgb_image, cv2.COLOR_BGR2HSV)
        lower_red = np.array([20, 20, 20])
        upper_red = np.array([200, 200, 200])
        mask = cv2.inRange(hsv, lower_red, upper_red)
        ...
        open_kernel = np.ones((5, 5), dtype=np.uint8)
        image_open = cv2.morphologyEx(np.array(image_close), cv2.MORPH_OPEN, open_kernel)
        bounding_boxes, rgb_contour, mask_contour = self.get_bb_tumor(image_open, rgb_image,
                                                                      tumor_gt_mask)

        rgb_bbox = self.draw_bb(rgb_image, bounding_boxes)
        rgb_bbox_split = self.split_bb(rgb_image, bounding_boxes, image_open)

        # cv2.imshow('contour', rgb_contour)
        # cv2.imshow('contour_mask', mask_contour)
        # cv2.imshow('bbox', rgb_bbox)
        # cv2.imshow('open_image', rgb_bbox_split)
        # cv2.imshow('mask', mask)
        # cv2.waitKey(0) & 0xFF

        return bounding_boxes, rgb_contour, mask_contour, rgb_bbox, image_open
    ...
    @staticmethod
    def get_bb_normal(cont_img, image):
    ...
    @staticmethod
    def get_bb_tumor(cont_img, image, tumor_gt_mask):
        rgb_contour = image.copy()
    ...
    @staticmethod
    def split_bb(image, bounding_boxes, image_open):
